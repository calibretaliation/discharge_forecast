...
from model.model import gwnet_new
from model.DSTAGNN import make_model
from model.ASTGCRN import ASTGCRN
...
class TrainingConfig:
    ...
    if DATASET == "camels":
        ...
        PROCESSED_DATA_DIR = Path(f"dataset/test_{MODEL}_processed_no_mask")
        # PRETRAIN_MODEL_PATH = "weights/camels/best_gwnet_model.pt"
        PRETRAIN_MODEL_PATH = "None" #Train from scratch
        BEST_MODEL_SAVE_PATH = f"weights/ours/test_{MODEL}_model_no_mask.pt"
        PLOT_SAVE_DIR = Path(f"plots/training/test_{MODEL}_no_mask")
...
def calculate_r_squared(y_true, y_pred):
    # Calculate the means of the true and predicted values
    mean_true = torch.mean(y_true)
    mean_pred = torch.mean(y_pred)

    # Calculate the numerator for the Pearson correlation coefficient (covariance)
    numerator = torch.sum((y_true - mean_true) * (y_pred - mean_pred))

    # Calculate the two terms for the denominator (square root of the product of variances)
    denom_true = torch.sum((y_true - mean_true)**2)
    denom_pred = torch.sum((y_pred - mean_pred)**2)
    
    # Calculate the Pearson correlation coefficient
    # Add a small epsilon to the denominator to avoid division by zero
    epsilon = 1e-8
    correlation = numerator / (torch.sqrt(denom_true * denom_pred) + epsilon)

    # Square the correlation to get R-squared and return as a standard Python float
    r_squared = correlation**2
    
    # Check for and print warnings for unusually large or infinite R-squared values.
    if (abs(r_squared) > 1e4) or (r_squared == float('inf')) or (r_squared == -float('inf')):
        print("Warning: R-squared value is unusually high or infinite.")
        print("y_true:", y_true)
        print("y_pred:", y_pred)
        print("numerator:", numerator)
        print("denominator:", (torch.sqrt(denom_true * denom_pred) + epsilon))
...
class Trainer:
    ...
    def eval_epoch(self):
        ...
        with torch.no_grad():
            for x_batch, y_batch, x_mask, y_mask in tqdm(self.val_loader, desc="Validating", leave=False):
                ...
                all_y_pred_scaled.append(output)
                all_y_true_scaled.append(y_target)
                all_x_true_scaled.append(x_permuted)

        all_y_true = torch.cat(all_y_true_unscaled, dim=0)
        all_y_pred = torch.cat(all_y_pred_unscaled, dim=0)
        batch_size, seq_len, num_nodes = all_y_true.shape
        valid_nses = []
        valid_r_squareds = []
        valid_rmses = []
        ...
        for i in range(num_nodes):
            ...
            node_r_squared = calculate_r_squared(y_true_node_i, y_pred_node_i)
            rmse = torch.sqrt(F.mse_loss(y_pred_node_i, y_true_node_i))
            valid_nses.append(node_nse)
            valid_r_squareds.append(node_r_squared)
        ...
        valid_r_squareds = [n for n in valid_r_squareds if abs(n) < 1e8]  # Filter out extreme NSE values
        average_r_squared = np.mean(valid_r_squareds) if valid_r_squareds else -np.inf
        valid_rmses = [n for n in valid_rmses if abs(n) < 1e8]  # Filter out extreme NSE values
        average_rmses = np.mean(valid_rmses) if valid_rmses else -np.inf
        
        return avg_loss, average_nse, valid_nses, average_r_squared, valid_r_squareds, average_rmses, valid_rmses, torch.cat(all_y_true_scaled, dim = 0), torch.cat(all_y_pred_scaled, dim = 0),torch.cat(all_x_true_scaled, dim = 0), all_y_mask
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            avg_val_loss, val_nse, valid_nses, val_r_squared, valid_r_squareds, average_rmses, valid_rmses, all_y_true, all_y_pred, all_x_true, all_y_mask = self.eval_epoch()
            # self.scheduler.step(avg_val_loss)
            self.train_loss_history.append(avg_train_loss)
            self.val_loss_history.append(avg_val_loss)
            self.val_nse_history.append(val_nse)
            self.val_r_squared_history.append(val_r_squared)

            print(f"Epoch {epoch}/{self.config.num_epochs} | "
                  f"Train Loss: {avg_train_loss:.4f} | "
                  f"Val Loss: {avg_val_loss:.4f} | "
                  f"Val NSE: {val_nse:.4f} | "
                  f"Val R-squared: {val_r_squared:.4f} | "
                  f"Val RMSE: {average_rmses:.4f}")
            ...
            if avg_val_loss < best_val_loss and not np.isnan(avg_val_loss):
                ...
                with open(self.config.PROCESSED_DATA_DIR / "valid_nses.txt", 'w') as f:
                    for nse in valid_nses:
                        f.write(f"{nse}\n")
                with open(self.config.PROCESSED_DATA_DIR / "valid_r_squareds.txt", 'w') as f:
                    for r2 in valid_r_squareds:
                        f.write(f"{r2}\n")

                # save y_true to npy
                np.save(self.config.PROCESSED_DATA_DIR / "y_true.npy", all_y_true.cpu().numpy())
                np.save(self.config.PROCESSED_DATA_DIR / "y_pred.npy", all_y_pred.cpu().numpy())
                np.save(self.config.PROCESSED_DATA_DIR / "x_true.npy", all_x_true.cpu().numpy())

                # save train loss history to npy
                np.save(self.config.PROCESSED_DATA_DIR / "train_loss_history.npy", np.array(self.train_loss_history))
                np.save(self.config.PROCESSED_DATA_DIR / "val_loss_history.npy", np.array(self.val_loss_history))
            self.plot_metrics()
